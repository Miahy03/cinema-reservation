...
class ProjectionIT {
  ...
  void put_projection_ok_for_manager() {
    ...
    var body =
        new UpsertProjection(
            projection.getId(), newDatetime, new BigDecimal("12.50"), movie.getId(), room.getId());
  ...
  void get_projections_ok_without_authentication() {
    var response =
        rest.exchange("/projections", HttpMethod.GET, HttpEntity.EMPTY, ProjectionResponse[].class);
